...
if(task.team.length  > 1){
  ...
  text = text + `The task priority is set a ${task.priority} priority, so check and act act accordingly. The task date is ${task.date.toDateString()}. 
  Thank you!!!`
...
export const dashboardStatistics = async(req, res) => {
    try {
        ...
        const users = await User.find({isActive: true}).select("name title role isAdmin createdAt")
        .limit(10)
        .sort({_id : -1})
